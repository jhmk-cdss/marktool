...
@Service
public class collection_type3Impl {
    @Autowired
    private MongoTemplate mongoTemplate;

    public void save(CollectionType3 clazz) {
        mongoTemplate.save(clazz);
    }

    public void delete(String id) {
        Query query = new Query(Criteria.where("_id").is(id));
        mongoTemplate.remove(query,CollectionType3.class);
    }
...
//        Update update= new Update().set("dept_name", bean.getDept_name()).set("passWord", user.getPassWord());
    ...
    public CollectionType3 findById(String id) {
        Query query = new Query(Criteria.where("_id").is(id));
        CollectionType3 user = mongoTemplate.findOne(query, CollectionType3.class);
        return user;
    }
}
